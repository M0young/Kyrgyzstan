...
_GL.CLASSIFICATION = (function () {
	...
	function handleCreateClassSave() {
		...
		if (!landClassCode) {
			_GL.COMMON.showAlert("createClassCodeAlert", "Enter Classification Code");
			return;
		}
		...
		for(let row of rows) {
			...
			if (existingCode === landClassCode){
				_GL.COMMON.showAlert("createClassCodeAlert", "Duplicate Classification Code");
			}
		}
		
		if(!landClassEn) {
			_GL.COMMON.showAlert("createClassEnAlert", "Enter Classification in English");
			return;
		}
		
		if(!landClassKy) {
			_GL.COMMON.showAlert("createClassKyAlert", "Enter Classification in kirghiz");
			return;
		}
		
		if(!landClassRu) {
			_GL.COMMON.showAlert("createClassRuAlert", "Enter Classificaiton in Russian");
			return;
		}
	...
	function setupEditEvent() {
		...
		elements.editsaveButton.addEventListener('click', function () {
			...
			if (!elements.editClassEn.value.trim()) {
				_GL.COMMON.showAlert("editClassEnAlert", "Enter Classification English", 'error');
				return;
			}
			
			if (!elements.editClassKy.value.trim()) {
				_GL.COMMON.showAlert("editClassKyAlert", "Enter Classification Kirghiz", 'error');
				return;
			}
			
			if (!elements.editClassRu.value.trim()) {
				_GL.COMMON.showAlert("editClassRuAlert", "Enter Classification Russian", 'error');
				return;
			}
