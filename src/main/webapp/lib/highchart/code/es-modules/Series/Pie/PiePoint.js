...
extend(PiePoint.prototype, {
    connectorShapes: {
        ...
        crookedLine: function (labelPosition, connectorPosition, options) {
            var touchingSliceAt = connectorPosition.touchingSliceAt, series = this.series, pieCenterX = series.center[0], plotWidth = series.chart.plotWidth, plotLeft = series.chart.plotLeft, alignment = labelPosition.alignment, radius = this.shapeArgs.r, crooklayerance = relativeLength(// % to fraction
            options.crooklayerance, 1), crookX = alignment === 'left' ?
                pieCenterX + radius + (plotWidth + plotLeft -
                    pieCenterX - radius) * (1 - crooklayerance) :
                plotLeft + (pieCenterX - radius) * crooklayerance, segmentWithCrook = [
                'L',
                crookX,
                labelPosition.y
            ];
