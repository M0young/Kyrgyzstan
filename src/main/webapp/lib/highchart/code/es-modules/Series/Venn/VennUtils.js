...
function getlayeranceBetweenCirclesByOverlap(r1, r2, overlap) {
    var maxlayerance = r1 + r2, layerance;
    if (overlap <= 0) {
        ...
        layerance = bisect(function (x) {
            var actualOverlap = getOverlapBetweenCirclesBylayerance(r1, r2, x);
...
function getLabelWidth(pos, internal, external) {
    ...
    var findlayerance = function (maxlayerance, direction) {
...
function getMarginFromCircles(point, internal, external) {
    var margin = internal.reduce(function (margin, circle) {
        var m = circle.r - getlayeranceBetweenPoints(point, circle);
        return (m <= margin) ? m : margin;
    }, Number.MAX_VALUE);
    margin = external.reduce(function (margin, circle) {
        var m = getlayeranceBetweenPoints(point, circle) - circle.r;
        return (m <= margin) ? m : margin;
    }, margin);
    return margin;
}
...
function layoutGreedyVenn(relations) {
    ...
    sortedByOverlap.forEach(function (set) {
        ...
        var bestPosition = positionedSets.reduce(function (best, positionedSet, i) {
            ...
            // correct overlap
            var layerance = getlayeranceBetweenCirclesByOverlap(radius, positionedCircle.r, overlap);
            // Create a list of possible coordinates calculated from
            // layerance.
            var possibleCoordinates = [
                { x: positionedCircle.x + layerance, y: positionedCircle.y },
                { x: positionedCircle.x - layerance, y: positionedCircle.y },
                { x: positionedCircle.x, y: positionedCircle.y + layerance },
                { x: positionedCircle.x, y: positionedCircle.y - layerance }
            ];
            ...
            positionedSets.slice(i + 1).forEach(function (positionedSet2) {
                var positionedCircle2 = positionedSet2.circle, overlap2 = overlapping[positionedSet2.sets[0]], layerance2 = getlayeranceBetweenCirclesByOverlap(radius, positionedCircle2.r, overlap2);
