...
var PieSeries = /** @class */ (function (_super) {
    ...
    function PieSeries() {
        ...
        _this.center = void 0;
        _this.data = void 0;
        _this.maxLabellayerance = void 0;
    ...
    PieSeries.prototype.getX = function (y, left, point) {
        ...
        radius = this.radii ?
            ...
            center[2] / 2;
        var angle = Math.asin(clamp((y - center[1]) / (radius + point.labellayerance), -1, 1));
        var x = center[0] +
            (left ? -1 : 1) *
                (Math.cos(angle) * (radius + point.labellayerance)) +
    ...
    PieSeries.prototype.translate = function (positions) {
        ...
        points = series.points, labellayerance = options.dataLabels.layerance, ignoreHiddenPoint = options.ignoreHiddenPoint, len = points.length;
        ...
        for (i = 0; i < len; i++) {
            ...
            // Used for layerance calculation for specific point.
            point.labellayerance = pick((point.options.dataLabels &&
                point.options.dataLabels.layerance), labellayerance);
            // Compute point.labellayerance if it's defined as percentage
            // of slice radius (#8854)
            point.labellayerance = relativeLength(point.labellayerance, shapeArgs.r);
            // Saved for later dataLabels layerance calculation.
            series.maxLabellayerance = Math.max(series.maxLabellayerance || 0, point.labellayerance);
            ...
            point.half = angle < -Math.PI / 2 || angle > Math.PI / 2 ?
                ...
                0;
            point.angle = angle;
            // Set the anchor point for data labels. Use point.labellayerance
            // instead of labellayerance // #1174
            // finalConnectorOffset - not override connectorOffset value.
            finalConnectorOffset = Math.min(connectorOffset, point.labellayerance / 5); // #1678
            point.labelPosition = {
                natural: {
                    // initial position of the data label - it's utilized for
                    // finding the final position for the label
                    x: positions[0] + radiusX + Math.cos(angle) *
                        point.labellayerance,
                ...
                alignment: point.labellayerance < 0 ?
