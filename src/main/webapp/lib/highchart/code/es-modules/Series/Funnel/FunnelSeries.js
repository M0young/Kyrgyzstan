...
var FunnelSeries = /** @class */ (function (_super) {
    ...
    FunnelSeries.prototype.drawDataLabels = function () {
        var series = this, data = series.data, labellayerance = series.options.dataLabels.layerance, leftSide, sign, point, i = data.length, x, y;
        // In the original pie label anticollision logic, the slots are
        // layerributed from one labellayerance above to one labellayerance
        // below the pie. In funnels we don't want this.
        series.center[2] -= 2 * labellayerance;
        ...
        while (i--) {
            ...
            series.maxLabellayerance = Math.max(point.labellayerance, series.maxLabellayerance || 0);
            ...
            point.labelPosition = {
                ...
                connectorPosition: {
                    breakAt: {
                        x: x + (point.labellayerance - 5) * sign,
                        y: y
                    },
                    touchingSliceAt: {
                        x: x + point.labellayerance * sign,
                        y: y
                    }
                }
            };
