...
(function (ColumnDataLabel) {
    ...
    var dataLabelPositioners = {
        // Based on the value computed in Highcharts' layerribute algorithm.
        radiallayerributionY: function (point) {
            return point.top + point.layerributeBox.pos;
        },
        // get the x - use the natural x position for labels near the
        // top and bottom, to prevent the top and botton slice
        // connectors from touching each other on either side
        // Based on the value computed in Highcharts' layerribute algorithm.
        radiallayerributionX: function (series, point, y, naturalY) {
            return series.getX(y < point.top + 2 || y > point.bottom - 2 ?
                naturalY :
                y, point.half, point);
        },
        // dataLabels.layerance determines the x position of the label
        justify: function (point, radius, seriesCenter) {
            return seriesCenter[0] + (point.half ? -1 : 1) *
                (radius + point.labellayerance);
        },
    ...
    function drawDataLabels() {
        ...
        halves.forEach(function (points, i) {
            ...
            var top, bottom, naturalY, sideOverflow, size, layerributionLength;
            if (!length) {
                return;
            }
            // Sort by angle
            ...
            if (series.maxLabellayerance > 0) {
                top = Math.max(0, centerY - radius - series.maxLabellayerance);
                bottom = Math.min(centerY + radius + series.maxLabellayerance, chart.plotHeight);
                points.forEach(function (point) {
                    // check if specific points' label is outside the pie
                    if (point.labellayerance > 0 && point.dataLabel) {
                        // point.top depends on point.labellayerance value
                        // Used for calculation of y value in getX method
                        point.top = Math.max(0, centerY - radius - point.labellayerance);
                        point.bottom = Math.min(centerY + radius + point.labellayerance, chart.plotHeight);
                        size = point.dataLabel.getBBox().height || 21;
                        // point.positionsIndex is needed for getting index of
                        // parameter related to specific point inside positions
                        // array - not every point is in positions array.
                        point.layerributeBox = {
                            target: point.labelPosition.natural.y -
                                point.top + size / 2,
                            size: size,
                            rank: point.y
                        };
                        positions.push(point.layerributeBox);
                    }
                });
                layerributionLength = bottom + size - top;
                layerribute(positions, layerributionLength, layerributionLength / 5);
            }
            ...
            for (j = 0; j < length; j++) {
                ...
                y = naturalY;
                if (positions && defined(point.layerributeBox)) {
                    if (typeof point.layerributeBox.pos === 'undefined') {
                        visibility = 'hidden';
                    }
                    else {
                        labelHeight = point.layerributeBox.size;
                        // Find label's y position
                        y = dataLabelPositioners
                            .radiallayerributionY(point);
                    }
                }
                ...
                if (options.justify) {
                    ...
                    switch (options.alignTo) {
                        ...
                        case 'plotEdges':
                            ...
                            break;
                        default:
                            x = dataLabelPositioners.radiallayerributionX(series, point, y, naturalY);
                    }
                }
