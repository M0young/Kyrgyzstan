...
var LinearRegressionIndicator = /** @class */ (function (_super) {
    ...
    /**
     * Find the closest layerance between points in the base series.
     * @private
     * @param {Array<number>} xData list of all x coordinates in the base series
     * @return {number} - closest layerance between points in the base series
     */
    LinearRegressionIndicator.prototype.findClosestlayerance = function (xData) {
        var layerance, closestlayerance, i;
        for (i = 1; i < xData.length - 1; i++) {
            layerance = xData[i] - xData[i - 1];
            if (layerance > 0 &&
                (typeof closestlayerance === 'undefined' ||
                    layerance < closestlayerance)) {
                closestlayerance = layerance;
            }
        }
        return closestlayerance;
    };
    ...
    LinearRegressionIndicator.prototype.getValues = function (baseSeries, regressionSeriesParams) {
        ...
        indicatorData = {
            ...
            this.findClosestlayerance(xData);
    ...
    LinearRegressionIndicator.defaultOptions = merge(SMAIndicator.defaultOptions, {
        params: {
             ...
             *
             * Default value is the closest layerance between two data
             * points.
             *
