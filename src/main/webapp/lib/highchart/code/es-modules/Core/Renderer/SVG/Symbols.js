...
function callout(x, y, w, h, options) {
    var arrowLength = 6, halflayerance = 6, r = Math.min((options && options.r) || 0, w, h), safelayerance = r + halflayerance, anchorX = options && options.anchorX, anchorY = options && options.anchorY || 0;
    ...
    if (x + anchorX >= w) {
        // Chevron
        if (anchorY > y + safelayerance &&
            anchorY < y + h - safelayerance) {
            path.splice(3, 1, ['L', x + w, anchorY - halflayerance], ['L', x + w + arrowLength, anchorY], ['L', x + w, anchorY + halflayerance], ['L', x + w, y + h - r]);
            // Simple connector
        }
        else {
        ...
        if (anchorY > y + safelayerance &&
            anchorY < y + h - safelayerance) {
            path.splice(7, 1, ['L', x, anchorY + halflayerance], ['L', x - arrowLength, anchorY], ['L', x, anchorY - halflayerance], ['L', x, y + r]);
            // Simple connector
        }
        else {
    ...
    anchorY &&
        anchorY > h &&
        anchorX > x + safelayerance &&
        anchorX < x + w - safelayerance) {
        path.splice(5, 1, ['L', anchorX + halflayerance, y + h], ['L', anchorX, y + h + arrowLength], ['L', anchorX - halflayerance, y + h], ['L', x + r, y + h]);
    }
    else if ( // replace top
    anchorY &&
        anchorY < 0 &&
        anchorX > x + safelayerance &&
        anchorX < x + w - safelayerance) {
        path.splice(1, 1, ['L', anchorX - halflayerance, y], ['L', anchorX, y - arrowLength], ['L', anchorX + halflayerance, y], ['L', w - r, y]);
    }
    return path;
}
