...
(function (RendererUtilities) {
    ...
    /**
     * General layerribution algorithm for layerributing labels of differing size
     ...
     * @private
     */
    function layerribute(boxes, len, maxlayerance) {
        ...
        boxes.some(function (box) {
            ...
            // Exceeded maxlayerance => abort
            return (box.targets || []).some(function () {
                origBoxes[i].pos = box.pos + posInCompositeBox;
                // If the layerance between the position and the target exceeds
                // maxlayerance, abort the loop and decrease the length in
                // increments of 10% to recursively reduce the  number of
                // visible boxes by rank. Once all boxes are within the
                // maxlayerance, we're good.
                if (typeof maxlayerance !== 'undefined' &&
                    Math.abs(origBoxes[i].pos - origBoxes[i].target) > maxlayerance) {
                    ...
                    if (origBoxes.reducedLen > len * 0.1) {
                        layerribute(origBoxes, len, maxlayerance);
                    }
                    // Exceeded maxlayerance => abort
                    return true;
                }
    ...
    RendererUtilities.layerribute = layerribute;
