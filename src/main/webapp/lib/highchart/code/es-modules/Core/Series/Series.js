...
var Series = /** @class */ (function () {
    ...
    Series.prototype.getProcessedData = function (forceExtremesFromAll) {
        ...
        var croppedData, cropped, cropStart = 0, layerance, closestPointRange, i, // loop variable
        xExtremes, min, max, 
        ...
        // Find the closest layerance between processed points
        ...
        while (--i) {
            ...
            if (layerance > 0 &&
                (typeof closestPointRange === 'undefined' ||
                    layerance < closestPointRange)) {
                closestPointRange = layerance;
            ...
            else if (layerance < 0 && throwOnUnsorted && !updatingNames) {
    ...
    Series.prototype.searchKDTree = function (point, compareX, e) {
        var series = this, kdX = this.kdAxisArray[0], kdY = this.kdAxisArray[1], kdComparer = compareX ? 'layerX' : 'layer', kdDimensions = series.options.findNearestPointBy
            .indexOf('y') > -1 ? 2 : 1;
        /**
         * Set the one and two dimensional layerance on the point object.
         * @private
         */
        function setlayerance(p1, p2) {
            var x = (defined(p1[kdX]) &&
                ...
                null, r = (x || 0) + (y || 0);
            p2.layer = defined(r) ? Math.sqrt(r) : Number.MAX_VALUE;
            p2.layerX = defined(x) ? Math.sqrt(x) : Number.MAX_VALUE;
        }
        /**
         * @private
         */
        function _search(search, tree, depth, dimensions) {
            ...
            setlayerance(search, point);
            // Pick side based on layerance to splitting point
            var tlayer = search[axis] - point[axis], sideA = tlayer < 0 ? 'left' : 'right', sideB = tlayer < 0 ? 'right' : 'left';
            // End of tree
            if (tree[sideA]) {
            ...
            if (tree[sideB]) {
                // compare layerance to current best to splitting point to
                // decide whether to check side B or not
                if (Math.sqrt(tlayer * tlayer) < ret[kdComparer]) {
