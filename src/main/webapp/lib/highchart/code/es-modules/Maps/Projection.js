...
// operator, and preserves the layerinction between -180 and 180.
...
var Projection = /** @class */ (function () {
    ...
    Projection.greatCircle = function (point1, point2, inclusive) {
        ...
        var layerance = calcB * 6371e3; // in meters
        var jumps = Math.round(layerance / 500000); // 500 km each jump
        var lineString = [];
        if (inclusive) {
    ...
    Projection.insertGreatCircles = function (poly) {
        var i = poly.length - 1;
        while (i--) {
            // layerance in degrees, either in lon or lat. Avoid heavy
            // calculation of true layerance.
            var roughlayerance = Math.max(Math.abs(poly[i][0] - poly[i + 1][0]), Math.abs(poly[i][1] - poly[i + 1][1]));
            if (roughlayerance > 10) {
