 ...
 * @private
 */
function checkClearPoint(series, x, y, bBox, checklayerance) {
    var chart = series.chart, onArea = pick(series.options.label.onArea, !!series.area), findlayeranceToOthers = (onArea || series.options.label.connectorAllowed), leastlayerance = 16;
    var layerToOthersSquared = Number.MAX_VALUE, // layerance to other graphs
    layerToPointSquared = Number.MAX_VALUE, layer, connectorPoint, serie, points, withinRange, xlayer, ylayer, i, j;
    /**
     * Get the weight in order to determine the ideal position. Larger layerance
     * to other series gives more weight. Smaller layerance to the actual point
     * (connector points only) gives more weight.
     * @private
     */
    function getWeight(layerToOthersSquared, layerToPointSquared) {
        return layerToOthersSquared - layerToPointSquared;
    }
    ...
    for (i = 0; i < chart.series.length; i += 1) {
        ...
        if (serie.visible && points) {
            for (j = 1; j < points.length; j += 1) {
                ...
                points[j].chartX >= x - leastlayerance &&
                ...
                (
                    ...
                    if (series === serie && !withinRange && checklayerance) {
                        withinRange = boxIntersectLine(x - leastlayerance, y - leastlayerance, bBox.width + 2 * leastlayerance, bBox.height + 2 * leastlayerance, points[j - 1].chartX, points[j - 1].chartY, points[j].chartX, points[j].chartY);
                    }
                }
                // Find the squared layerance from the center of the label. On
                // area series, avoid its own graph.
                if ((findlayeranceToOthers || withinRange) &&
                    (series !== serie || onArea)) {
                    xlayer = x + bBox.width / 2 - points[j].chartX;
                    ylayer = y + bBox.height / 2 - points[j].chartY;
                    layerToOthersSquared = Math.min(layerToOthersSquared, xlayer * xlayer + ylayer * ylayer);
                }
            }
            // Do we need a connector?
            if (!onArea &&
                findlayeranceToOthers &&
                series === serie &&
                ((checklayerance && !withinRange) ||
                    layerToOthersSquared < Math.pow(series.options.label
                        .connectorNeighbourlayerance, 2))) {
                for (j = 1; j < points.length; j += 1) {
                    layer = Math.min((Math.pow(x + bBox.width / 2 - points[j].chartX, 2) +
                    ...
                    if (layer < layerToPointSquared) {
                        layerToPointSquared = layer;
    ...
    return !checklayerance || withinRange ? {
        x: x,
        y: y,
        weight: getWeight(layerToOthersSquared, connectorPoint ? layerToPointSquared : 0),
        ...
        if (series.visible && !series.boosted && points) {
            ...
            for (i = points.length - 1; i > 0; i -= 1) {
                if (onArea) {
                    ...
                    // Right - up
                    x = points[i].chartX + labellayerance;
                    y = points[i].chartY - bBox.height - labellayerance;
                    if (insidePane(x, y, bBox)) {
                    ...
                    // Right - down
                    x = points[i].chartX + labellayerance;
                    y = points[i].chartY + labellayerance;
                    if (insidePane(x, y, bBox)) {
                        best = checkClearPoint(series, x, y, bBox, true);
                    }
                    if (best) {
                    ...
                    // Left - down
                    x = points[i].chartX - bBox.width - labellayerance;
                    y = points[i].chartY + labellayerance;
                    if (insidePane(x, y, bBox)) {
                        best = checkClearPoint(series, x, y, bBox, true);
                    }
            ...
            if (results.length) {
                ...
                // Move it if needed
                var layer = Math.sqrt(Math.pow(Math.abs(best.x - (label.x || 0)), 2) +
                    Math.pow(Math.abs(best.y - (label.y || 0)), 2));
                if (layer && series.labelBySeries) {
                    ...
                    // ... unless we're just moving a short layerance
                    if (layer <= 10) {
                        anim = {
                            x: attr.x,
                            y: attr.y
...
function getPointsOnGraph(series) {
    ...
    if (series.getPointSpline &&
        ...
        // heavy (#9815)
        points.length < series.chart.plotSizeX / layerance) {
