...
var clusterDefaultOptions = {
    ...
    layoutAlgorithm: {
         ...
         * first step, points are divided using the grid method (layerance
         * property is a grid size) to find the initial amount of clusters.
         * Next, each point is classified by computing the layerance between
         * each cluster center and that point. When the closest cluster
         * layerance is lower than layerance property set by a user the point
         ...
         * layerance from the cluster center and rebuild it when the point is
        ...
        /**
         * When `type` is set to `kmeans`,
         * `layerance` is a maximum layerance between point and cluster center
         * so that this point will be inside the cluster. The layerance
...
Scatter.prototype.getClusterlayerancesFromPoint = function (clusters, pointX, pointY) {
    var pointClusterlayerance = [];
    for (var clusterIndex = 0; clusterIndex < clusters.length; clusterIndex++) {
        ...
        }), layerance = Math.sqrt(Math.pow(p1.x - p2.x, 2) +
            Math.pow(p1.y - p2.y, 2));
        pointClusterlayerance.push({ clusterIndex: clusterIndex, layerance: layerance });
    }
    return pointClusterlayerance.sort(function (a, b) { return a.layerance - b.layerance; });
};
...
Scatter.prototype.markerClusterAlgorithms = {
    ...
    kmeans: function (dataX, dataY, dataIndexes, options) {
        var series = this, clusters = [], noise = [], group = {}, pointMaxlayerance = options.processedlayerance ||
            clusterDefaultOptions.layoutAlgorithm.layerance, iterations = options.iterations, 
        // Max pixel difference beetwen new and old cluster position.
        maxClusterShift = 1, currentIteration = 0, repeat = true, pointX = 0, pointY = 0, tempPos, pointClusterlayerance = [], groupedData, key, i, j;
        options.processedGridSize = options.processedlayerance;
        ...
        while (repeat) {
            ...
            for (i = 0; i < dataX.length; i++) {
                pointX = dataX[i];
                pointY = dataY[i];
                pointClusterlayerance = series.getClusterlayerancesFromPoint(clusters, pointX, pointY);
                if (pointClusterlayerance.length &&
                    pointClusterlayerance[0].layerance < pointMaxlayerance) {
                    clusters[pointClusterlayerance[0].clusterIndex].points.push({
                        x: pointX,
                        y: pointY,
                        dataIndex: dataIndexes[i]
                    });
            ...
            for (j = 0; j < clusters.length; j++) {
                if (clusters[j].points.length === 1) {
                    pointClusterlayerance = series.getClusterlayerancesFromPoint(clusters, clusters[j].points[0].x, clusters[j].points[0].y);
                    if (pointClusterlayerance[1].layerance < pointMaxlayerance) {
                        // Add point to the next closest cluster.
                        clusters[pointClusterlayerance[1].clusterIndex].points
                            .push(clusters[j].points[0]);
                        // Clear points array.
                        clusters[pointClusterlayerance[0].clusterIndex]
                            .points.length = 0;
                    }
                }
            }
    ...
    optimizedKmeans: function (processedXData, processedYData, dataIndexes, options) {
        var series = this, pointMaxlayerance = options.processedlayerance ||
            clusterDefaultOptions.layoutAlgorithm.gridSize, group = {}, extremes = series.getRealExtremes(), clusterMarkerOptions = (series.options.cluster || {}).marker, offset, layerance, radius;
        if (!series.markerClusterInfo || (series.initMaxX && series.initMaxX < extremes.maxX ||
            ...
            series.baseClusters.clusters.forEach(function (cluster) {
                ...
                cluster.data.forEach(function (dataPoint) {
                    ...
                    layerance = Math.sqrt(Math.pow(dataPointPx.x - clusterPx.x, 2) +
                    ...
                    offset = pointMaxlayerance - radius >= 0 ?
                        pointMaxlayerance - radius : radius;
                    if (layerance > radius + offset &&
...
Scatter.prototype.preventClusterCollisions = function (props) {
    ...
    gridsToCheckCollision.forEach(function (item) {
        ...
        if (groupedData[item]) {
            ...
            maxlayer = clusterRadius + radius;
            radius = 0;
            if (itemX !== gridX &&
                Math.abs(xPixel - nextXPixel) < maxlayer) {
                xPixel = itemX - gridX < 0 ? gridXPx + clusterRadius :
                    gridXPx + gridSize - clusterRadius;
            }
            if (itemY !== gridY &&
                Math.abs(yPixel - nextYPixel) < maxlayer) {
...
Scatter.prototype.generatePoints = function () {
    ...
    if (clusterOptions &&
        ...
        layoutAlgOptions.processedlayerance = relativeLength(layoutAlgOptions.layerance ||
            clusterDefaultOptions.layoutAlgorithm.layerance, chart.plotWidth);
