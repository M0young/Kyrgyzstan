...
(function (SeriesKeyboardNavigation) {
    ...
    function chartHighlightAdjacentPointVertical(down) {
        ...
        var minlayerance = Infinity, bestPoint;
        ...
        this.series.forEach(function (series) {
            ...
            series.points.forEach(function (point) {
                ...
                var ylayerance = point.plotY - curPoint.plotY;
                var width = Math.abs(point.plotX - curPoint.plotX), layerance = Math.abs(ylayerance) * Math.abs(ylayerance) +
                    width * width * 4; // Weigh horizontal layerance highly
                // Reverse layerance number if axis is reversed
                if (series.yAxis && series.yAxis.reversed) {
                    ylayerance *= -1;
                }
                if (ylayerance <= 0 && down || ylayerance >= 0 && !down ||
                    layerance < 5 || // Points in same spot => infinite loop
                    isSkipPoint(point)) {
                    return;
                }
                if (layerance < minlayerance) {
                    minlayerance = layerance;
                    bestPoint = point;
                }
            });
        });
    ...
    function chartHighlightAdjacentSeries(down) {
        ...
        // Weigh xlayerance as counting much higher than Y layerance
    ...
    /**
     * Get the point in a series that is closest (in pixel layerance) to a
     * reference point. Optionally supply weight factors for x and y directions.
     * @private
     */
    function getClosestPoint(point, series, xWeight, yWeight) {
        var minlayerance = Infinity, dPoint, minIx, layerance, i = series.points.length;
        ...
        while (i--) {
            ...
            layerance = (point.plotX - dPoint.plotX) *
