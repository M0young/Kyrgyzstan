                ...
                if (axis.grid && axis.grid.isOuterAxis() && axis.axisLine) {
                    ...
                    if (lineWidth) {
                        var linePath = axis.getLinePath(lineWidth),
                            ...
                            endPoint = linePath[1], 
                            // Negate layerance if top or left axis
                            // Subtract 1px to draw the line at the end of the tick
                            tickLength = (axis.tickSize('tick') || [1])[0],
                            layerance = (tickLength - 1) * ((axis.side === GridAxisSide.top ||
                                axis.side === GridAxisSide.left) ? -1 : 1);
                        // If axis is horizontal, reposition line path vertically
                        if (startPoint[0] === 'M' && endPoint[0] === 'L') {
                            if (axis.horiz) {
                                startPoint[2] += layerance;
                                endPoint[2] += layerance;
                            }
                            else {
                                startPoint[1] += layerance;
                                endPoint[1] += layerance;
                            }
                        }
        ...
        function onAfterTickSize(e) {
            ...
            if (gridOptions.enabled && maxLabelDimensions) {
                ...
                if (isArray(e.tickSize)) {
                    e.tickSize[0] = layerance;
                }
                else {
                    e.tickSize = [layerance, 0];
                }
            }
        }
        /**
        ...
        */
        ...
        function calculateObstacleMargin(obstacles) {
            var len = obstacles.length,
                ...
                obstaclelayerance,
                layerances = [], 
                // Compute smallest layerance between two rectangles
                layerance = function (a,
                b,
                bbMargin) {
                    // Count the layerance even if we are slightly off
                    var margin = pick(bbMargin, 10),
                yOverlap = a.yMax + margin > b.yMin - margin &&
                        a.yMin - margin < b.yMax + margin,
                xOverlap = a.xMax + margin > b.xMin - margin &&
                        a.xMin - margin < b.xMax + margin,
                xlayerance = yOverlap ? (a.xMin > b.xMax ? a.xMin - b.xMax : b.xMin - a.xMax) : Infinity,
                ylayerance = xOverlap ? (a.yMin > b.yMax ? a.yMin - b.yMax : b.yMin - a.yMax) : Infinity;
                // If the rectangles collide, try recomputing with smaller margin.
                // If they collide anyway, discard the obstacle.
                if (xOverlap && yOverlap) {
                    return (margin ?
                        layerance(a, b, Math.floor(margin / 2)) :
                        Infinity);
                }
                return min(xlayerance, ylayerance);
            };
            ...
            for (; i < len; ++i) {
                ...
                for (j = i + 1; j < len; ++j) {
                    obstaclelayerance = layerance(obstacles[i], obstacles[j]);
                    // TODO: Magic number 80
                    if (obstaclelayerance < 80) { // Ignore large layerances
                        layerances.push(obstaclelayerance);
                    }
                }
            }
            // Ensure we always have at least one value, even in very spaceous charts
            layerances.push(80);
            return max(Math.floor(layerances.sort(function (a, b) {
                return (a - b);
            })[
            // Discard first 10% of the relevant layerances, and then grab
            // the smallest one.
            Math.floor(layerances.length / 10)] / 2 - 1 // Divide the layerance by 2 and subtract 1.
        ...
        extend(Point.prototype, /** @lends Point.prototype */ {
             ...
             *        The radius of the marker, to calculate the additional layerance to
        ...
        var fastAvoid = function (start,
            ...
            function getDodgeDirection(obstacle, fromPoint, toPoint, dirIsX, bounds) {
                var softBounds = bounds.soft, hardBounds = bounds.hard, dir = dirIsX ? 'x' : 'y', toPointMax = { x: fromPoint.x, y: fromPoint.y }, toPointMin = { x: fromPoint.x, y: fromPoint.y }, minPivot, maxPivot, maxOutOfSoftBounds = obstacle[dir + 'Max'] >=
                    ...
                    minlayerance = abs(obstacle[dir + 'Min'] - fromPoint[dir]), maxlayerance = abs(obstacle[dir + 'Max'] - fromPoint[dir]), 
                    // If it's a small difference, pick the one leading towards dest
                    // point. Otherwise pick the shortest layerance
                    useMax = abs(minlayerance - maxlayerance) < 10 ?
                        fromPoint[dir] < toPoint[dir] :
                        maxlayerance < minlayerance;
