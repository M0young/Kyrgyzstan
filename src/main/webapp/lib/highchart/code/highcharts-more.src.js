...
(function (factory) {
    ...
    _registerModule(_modules, 'Series/Bubble/BubbleLegendDefaults.js', [], function () {
        ...
        var BubbleLegendDefaults = {
            ...
            can be also defined
                 ...
                 * centered,
            the layerance is reduced to 0.
                 *
                 * @sample highcharts/bubble-legend/connectorandlabels/
                 *         Increased connector length
                 */
                connectorlayerance: 60,
                /**
    ...
    _registerModule(_modules, 'Series/Bubble/BubbleLegendItem.js', [_modules['Core/Color/Color.js'], _modules['Core/FormatUtilities.js'], _modules['Core/Globals.js'], _modules['Core/Utilities.js']], function (Color, F, H, U) {
        ...
        var BubbleLegendItem = /** @class */ (function () {
            ...
            BubbleLegendItem.prototype.drawLegendSymbol = function (legend) {
                var chart = this.chart,
                    itemlayerance = pick(legend.options.itemlayerance, 20),
                    legendItem = this.legendItem || {},
                    options = this.options,
                    ranges = options.ranges,
                    connectorlayerance = options.connectorlayerance;
                var connectorSpace;
                ...
                connectorSpace =
                    connectorlayerance - radius + maxLabel.width;
                connectorSpace = connectorSpace > 0 ? connectorSpace : 0;
                this.maxLabel = maxLabel;
                this.movementX = options.labels.align === 'left' ?
                    connectorSpace : 0;
                legendItem.labelWidth = size + connectorSpace + itemlayerance;
            ...
            BubbleLegendItem.prototype.renderRange = function (range) {
                var mainRange = this.ranges[0],
                    ...
                    connectorlayerance = options.connectorlayerance || 0,
                ...
                var connectorLength = rtl || labelsAlign === 'left' ?
                        -connectorlayerance : connectorlayerance;
                // Set options for centered labels
                if (labelsAlign === 'center') {
                    connectorLength = 0; // do not use connector
                    options.connectorlayerance = 0;
        ...
        var ColumnRangeSeries = /** @class */ (function (_super) {
            ...
            ColumnRangeSeries.prototype.translate = function () {
                ...
                var yAxis = this.yAxis,
                    ...
                    safelayerance = Math.max(chart.chartWidth,
                ...
                function safeBounds(pixelPos) {
                    return clamp(pixelPos, -safelayerance, safelayerance);
                }
            ...
            ColumnPyramidSeries.prototype.translate = function () {
                ...
                series.points.forEach(function (point) {
                    var yBottom = pick(point.yBottom,
                        translatedThreshold),
                        safelayerance = 999 + Math.abs(yBottom),
                        plotY = clamp(point.plotY, -safelayerance,
                        yAxis.len + safelayerance), 
    ...
    _registerModule(_modules, 'Series/PackedBubble/PackedBubbleSeriesDefaults.js', [_modules['Core/Utilities.js']], function (U) {
        ...
        var PackedBubbleSeriesDefaults = {
            ...
            layoutAlgorithm: {
                ...
                /**
                 * The layerance between two bubbles, when the algorithm starts to
                 * treat two bubbles as overlapping. The `bubblePadding` is also the
                 * expected layerance between all the bubbles on simulation end.
                 */
                bubblePadding: 5,
                /**
    ...
    _registerModule(_modules, 'Series/Networkgraph/VerletIntegration.js', [], function () {
         ...
         * @param {Highcharts.PositionObject} layerance
         *        layerance between two nodes e.g. `{x, y}`
         */
        function attractive(link, force, layeranceXY) {
            var massFactor = link.getMass(),
                translatedX = -layeranceXY.x * force * this.diffTemperature,
                translatedY = -layeranceXY.y * force * this.diffTemperature;
         ...
         *
         * @private
         * @param {number} d current layerance between two nodes
         * @param {number} k expected layerance between two nodes
         * @return {number} force
         */
        ...
        /**
         * Estiamte the best possible layerance between two nodes, making graph
         * readable.
         * @private
         */
         ...
         *        layerance between two nodes e.g. `{x, y}`
         */
        function repulsive(node, force, layeranceXY) {
            var factor = force * this.diffTemperature / node.mass / node.degree;
            if (!node.fixedPosition) {
                node.plotX += layeranceXY.x * factor;
                node.plotY += layeranceXY.y * factor;
            }
        }
        /**
    ...
    _registerModule(_modules, 'Series/PackedBubble/PackedBubbleIntegration.js', [_modules['Core/Globals.js'], _modules['Series/Networkgraph/VerletIntegration.js']], function (H, VerletIntegration) {
         ...
         * @private
         */
        function repulsive(node, force, layeranceXY, repNode) {
            var factor = (force * this.diffTemperature / node.mass /
                    node.degree),
                x = layeranceXY.x * factor,
                y = layeranceXY.y * factor;
            if (!node.fixedPosition) {
                node.plotX += x;
                node.plotY += y;
            }
    ...
    _registerModule(_modules, 'Series/Networkgraph/EulerIntegration.js', [], function () {
         ...
         * @param {number} force
         *        Force calcualated in `repulsiveForceFunction`
         * @param {Highcharts.PositionObject} layeranceXY
         *        layerance between two nodes e.g. `{x, y}`
         * @param {number} layeranceR
             */
        function attractive(link, force, layeranceXY, layeranceR) {
            var massFactor = link.getMass(),
                translatedX = (layeranceXY.x / layeranceR) * force,
                translatedY = (layeranceXY.y / layeranceR) * force;
            if (!link.fromNode.fixedPosition) {
                link.fromNode.dispX -=
                    translatedX * massFactor.fromNode / link.fromNode.degree;
                link.fromNode.dispY -=
        ...
        function integrate(layout, node) {
            ...
            layeranceR = node.temperature = layout.vectorLength({
                x: node.dispX,
                y: node.dispY
            });
            if (layeranceR !== 0) {
                node.plotX += (node.dispX / layeranceR *
                    Math.min(Math.abs(node.dispX), layout.temperature));
                node.plotY += (node.dispY / layeranceR *
         ...
         *        layerance between two nodes e.g. `{x, y}`
         */
        function repulsive(node, force, layeranceXY, layeranceR) {
            node.dispX +=
                (layeranceXY.x / layeranceR) * force / node.degree;
            node.dispY +=
                (layeranceXY.y / layeranceR) * force / node.degree;
        }
        /**
         * Repulsive force funtion. Can be replaced by API's
         * `layoutAlgorithm.repulsiveForce`.
    ...
    _registerModule(_modules, 'Series/Networkgraph/ReingoldFruchtermanLayout.js', [_modules['Series/Networkgraph/EulerIntegration.js'], _modules['Core/Globals.js'], _modules['Series/GraphLayoutComposition.js'], _modules['Series/Networkgraph/QuadTree.js'], _modules['Core/Utilities.js'], _modules['Series/Networkgraph/VerletIntegration.js']], function (EulerIntegration, H, GraphLayout, QuadTree, U, VerletIntegration) {
        ...
        var ReingoldFruchtermanLayout = /** @class */ (function () {
            ...
            ReingoldFruchtermanLayout.prototype.setK = function () {
                // Optimal layerance between nodes,
            ...
            ReingoldFruchtermanLayout.prototype.barnesHutApproximation = function (node, quadNode) {
                var layeranceXY = this.getlayerXY(node,
                    quadNode),
                    layeranceR = this.vectorLength(layeranceXY);
                var goDeeper,
                    force;
                if (node !== quadNode && layeranceR !== 0) {
                    if (quadNode.isInternal) {
                        // Internal node:
                        if (quadNode.boxSize / layeranceR <
                            this.options.theta &&
                            layeranceR !== 0) {
                            // Treat as an external node:
                            force = this.repulsiveForce(layeranceR, this.k);
                            this.force('repulsive', node, force * quadNode.mass, layeranceXY, layeranceR);
                            goDeeper = false;
                        }
                        else {
                            // Go deeper:
                    ...
                    for (var _b = 0, _c = this.nodes; _b < _c.length; _b++) {
                        ...
                        for (var _d = 0, _e = this.nodes; _d < _e.length; _d++) {
                            ...
                            node !== repNode &&
                                ...
                                // layout.getlayerR(node, repNode) < 2 * k &&
                                // Not dragged:
                                !node.fixedPosition) {
                                layeranceXY = this.getlayerXY(node, repNode);
                                layeranceR = this.vectorLength(layeranceXY);
                                if (layeranceR !== 0) {
                                    force = this.repulsiveForce(layeranceR, this.k);
                                    this.force('repulsive', node, force * repNode.mass, layeranceXY, layeranceR);
                                }
                            }
                        }
                    }
            ...
            ReingoldFruchtermanLayout.prototype.attractiveForces = function () {
                ...
                for (var _i = 0, _a = this.links; _i < _a.length; _i++) {
                    ...
                    if (link.fromNode && link.toNode) {
                        layeranceXY = this.getlayerXY(link.fromNode, link.toNode);
                        layeranceR = this.vectorLength(layeranceXY);
                        if (layeranceR !== 0) {
                            force = this.attractiveForce(layeranceR, this.k);
                            this.force('attractive', link, force, layeranceXY, layeranceR);
                        }
                    }
                }
            };
            ...
            ReingoldFruchtermanLayout.prototype.getlayerR = function (nodeA, nodeB) {
                var layerance = this.getlayerXY(nodeA,
                    nodeB);
                return this.vectorLength(layerance);
            };
            ReingoldFruchtermanLayout.prototype.getlayerXY = function (nodeA, nodeB) {
                var xlayer = nodeA.plotX - nodeB.plotX,
                    ylayer = nodeA.plotY - nodeB.plotY;
                return {
                    x: xlayer,
                    y: ylayer,
                    absX: Math.abs(xlayer),
                    absY: Math.abs(ylayer)
                };
            };
    ...
    _registerModule(_modules, 'Series/PackedBubble/PackedBubbleLayout.js', [_modules['Series/GraphLayoutComposition.js'], _modules['Series/PackedBubble/PackedBubbleIntegration.js'], _modules['Series/Networkgraph/ReingoldFruchtermanLayout.js'], _modules['Core/Utilities.js']], function (GraphLayout, PackedBubbleIntegration, ReingoldFruchtermanLayout, U) {
        ...
        /**
         * Remove accumulated data points to relayerribute all of them again
        ...
        var PackedBubbleLayout = /** @class */ (function (_super) {
                ...
                layout.nodes.forEach(function (node) {
                    ...
                    layout.nodes.forEach(function (repNode) {
                        ...
                        node !== repNode &&
                            ...
                            layeranceXY = layout.getlayerXY(node, repNode);
                            layeranceR = (layout.vectorLength(layeranceXY) -
                                (node.marker.radius +
                            ...
                            if (layeranceR < 0) {
                                node.degree += 0.01;
                                node.neighbours++;
                                force = layout.repulsiveForce(-layeranceR / Math.sqrt(node.neighbours), layout.k, node, repNode);
                            }
                            layout.force('repulsive', node, force * repNode.mass, layeranceXY, repNode, layeranceR);
                        }
                    });
                });
            };
            PackedBubbleLayout.prototype.applyLimitBox = function (node, box) {
                ...
                if (layout.options.splitSeries &&
                    ...
                    layeranceXY = layout.getlayerXY(node, node.series.parentNode);
                    layeranceR = (node.series.parentNodeRadius -
                        node.marker.radius -
                        layout.vectorLength(layeranceXY));
                    if (layeranceR < 0 &&
                        layeranceR > -2 * node.marker.radius) {
                        node.plotX -= layeranceXY.x * factor;
                        node.plotY -= layeranceXY.y * factor;
                    }
                }
        ...
        var PackedBubbleSeries = /** @class */ (function (_super) {
                    ...
                    if (parentNodeLayout && layout_1.options.dragBetweenSeries) {
                        parentNodeLayout.nodes.forEach(function (node) {
                            if (point && point.marker &&
                                ...
                                layeranceXY_1 = layout_1.getlayerXY(point, node);
                                layeranceR_1 = (layout_1.vectorLength(layeranceXY_1) -
                                    node.marker.radius -
                                    point.marker.radius);
                                if (layeranceR_1 < 0) {
            ...
            PackedBubbleSeries.prototype.positionBubble = function (lastBubble, newOrigin, nextBubble) {
                var sqrt = Math.sqrt,
                    ...
                    pow = Math.pow,
                    abs = Math.abs,
                    layerance = sqrt(// layer between lastBubble and newOrigin
                    ...
                    (pow(layerance, 2) +
                        ...
                        pow(nextBubble[2] + lastBubble[2], 2)) / (2 * (nextBubble[2] + newOrigin[2]) * layerance)),
    ...
    _registerModule(_modules, 'Core/Axis/RadialAxis.js', [_modules['Core/Axis/AxisDefaults.js'], _modules['Core/Defaults.js'], _modules['Core/Globals.js'], _modules['Core/Utilities.js']], function (AxisDefaults, D, H, U) {
        ...
        (function (RadialAxis) {
            ...
            /**
             * Find the center position of the label based on the layerance option.
             */
            function onTickAfterGetLabelPosition(e) {
                ...
                if (axis.isRadial) { // Both X and Y axes in a polar chart
                    ret = axis.getPosition(this.pos, (axis.center[2] / 2) +
                        relativeLength(pick(labelOptions.layerance, -25), axis.center[2] / 2, -axis.center[2] / 2));
    ...
    _registerModule(_modules, 'Series/PolarComposition.js', [_modules['Core/Animation/AnimationUtilities.js'], _modules['Core/Globals.js'], _modules['Extensions/Pane.js'], _modules['Core/Axis/RadialAxis.js'], _modules['Core/Utilities.js']], function (A, H, Pane, RadialAxis, U) {
         ...
         * and (yAxis.len - plotY) is the pixel layerance from center.
         * @private
         */
        function onSeriesAfterTranslate() {
            if (this.chart.polar && this.xAxis) {
                ...
                // case of shared tooltip, and by two dimensional layerance in case
                // of non-shared.
